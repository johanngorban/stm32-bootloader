...
Схему памяти см. в соответствующей директории документации: [docs/ru/memory_map](docs/ru/memory_map)
...
1. Быть собрана для запуска с адреса слота.
...
В `MEMORY` указывается начало слота и его длина. В ORIGIN поставьте адрес начала слота, в который будет заливаться код. В переменную LENGTH занесите размер слота:
